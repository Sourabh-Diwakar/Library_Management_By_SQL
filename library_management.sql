...
ALTER TABLE books
ALTER COLUMN category VARCHAR(20); -- Or a length that suits your needs
...
-------------------  return status table---
 ...
 alter table issued_status
 Add constraint fk_books foreign key (issued_book_isbn) references books(isbn_);


  alter table issued_status
 Add constraint fk_employe foreign key (issued_emp_id) references employe(emp_id);
 ...
 Add constraint fk_issued foreign key (issued_id) references issued_status(issued_id);
...
INSERT INTO employe(emp_id, emp_name, position, salary, branch_id) 
VALUES
...
INSERT INTO books(isbn_, book_title, category, rental_price, status_, author, publisher) 
VALUES
...
ALTER TABLE return_status
ADD book_quality VARCHAR(15) DEFAULT 'Good'; -- Corrected syntax
...
------fetch the data from the all tables
